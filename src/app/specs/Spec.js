...
class ArgumentSpecError extends Error {
  constructor(value, fName = "obj prop", specNum = 0, argNum = 0) {
    super("Argument error: " + fName + " argument #" + argNum +
      " (" + value + " : " + typeof value + ") violated spec #" + specNum);
  }
}

class ResultSpecError extends Error {
  constructor(value, fName, specNum = 0) {
    super("Result error: " + fName + " result (" + value + " : " + typeof value +
      ") violated spec #" + specNum);
  }
}

const mkError = (type, value, fName, specNum, argNum) => new ((type === "post") ? ResultSpecError : ArgumentSpecError)(
  value, fName, specNum, argNum
);
...
const specArrayOf = (specs) => (vals) => f.isArray(vals)
  && doto(
    vals.map((entry, pos) => checkSpecs("array-val", "array-val")([specs, entry], pos)),
    f.every(f.identity)
  );
...
const withSpecs = ({pre, post}) => (fn) => (!isDevel)
  ? fn
  : (...args) => {
    const preSpecs = (f.isArray(pre)) ? pre : [pre];
    const mandatoryArgs = doto(preSpecs,
      f.reject(f.get("optional")),
      f.size
    );

    const funcName = doto(fn,
      f.get("name"),
      (n) => (f.isEmpty(n)) ? "lambda" : n
    );
    ...
    if (!f.isNil(pre)) {
      ...
      if (argSize < mandatoryArgs) {
        ...
        console.warn(argSize - specSize + " additional arguments provided to " + funcName);
      }
    }

    const argsSuccess = f.isNil(pre)
      || (f.size(args) >= mandatoryArgs
        && doto(f.zip(preSpecs, args),
