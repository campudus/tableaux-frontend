...
const parseUrlFilterProp = props => {
  ...
  const primaryLangAsksForTranslation = (filter = {}) =>
    filter.mode === FilterModes.UNTRANSLATED &&
    props.langtag === f.first(Langtags);

  const checkFilter = f.cond([
    [f.eq(true), f.always({ mode: FilterModes.ID_ONLY, value: [props.rowId] })],
    [
      primaryLangAsksForTranslation,
      f.assoc("mode", FilterModes.ANY_UNTRANSLATED)
    ],
...
const updateFilter = (tableId, state, settings = {}, shouldSave) => {
  ...
  const isSortingEmpty =
    !f.isFinite(sorting.columnId) && f.isEmpty(sorting.value);
  const areAllFiltersEmpty =
    f.isEmpty(filters) || f.every(isFilterEmpty, filters);
  ...
  if (areAllFiltersEmpty && isSortingEmpty) {
    ...
    return f.assoc(["projection", "rows"], {}, state);
  } else {
    const rowsFilter = {
    ...
    return f.assoc(["projection", "rows"], rowsFilter, state);
  }
};
