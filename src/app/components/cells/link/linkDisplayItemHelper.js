...
const mkLinkDisplayItem = f.curry(function(table, row) {
  const rowData = f.isString(row) ? JSON.parse(row) : row;
  const cellData =
    f.get(["rows", "values"], rowData) || f.get("values", rowData);
  ...
  const addId = cell =>
    f.assoc(
      "id",
      `cell-${tableId}-${f.get("id", cell.column)}-${f.get("id", rowData)}`,
      cell
    );
...
const fetchLinkDisplayItem = f.curry(function(table, row) {
  const tableId = table.id;
  const rowId = row.id;
  return new Promise(function(resolve, reject) {
    Request.get(apiUrl(`/tables/${tableId}/rows/${rowId}`)).end(function(
      err,
      response
    ) {
      if (err) {
