...
const getDisplayValueOld = f.curryN(2)((column = {}, value) => {
...
const getLinkValue = column =>
  f.map(
    f.flow(
      f.get("value"),
      getDisplayValueOld(column.toColumn)
    )
  );
...
const getConcatValue = selector => column => value => {
  ...
  const displayValues = f.flatten(
    ...
    f.map(([col, val]) => getDisplayValueOld(col, val), concats)
  );
...
const format = f.curryN(2)((column, displayValue) => {
  if (f.isEmpty(f.get("formatPattern", column))) {
...
const tests = {
  ...
  tests: [
    ...
    [
      ...
      format,
      [
        { formatPattern: "{{1}} bar {{2}}", ...defaultTestGroups },
        ["foo", "baz"]
      ]
    ],
    [
...
export { format, tests };
export default getDisplayValueOld;
