...
var Infinite = React.createClass({
  ...
  getDefaultProps : function getDefaultProps() {
    return {
      ...
      className : '',
      //delay time the scroll function will be called in ms
      scrollDelayTime : 50,
    };
  },
  ...
  infiniteHandleScroll : function infiniteHandleScroll(e) {
    var self = this;
    var simpleEvent = {}; // we get all event values null when passing it inside the setTimeout Function so we save reference of target
    simpleEvent.target = e.target;
    if (this.utils.scrollDelayTimer) {
      clearTimeout(this.utils.scrollDelayTimer);
    }
    this.utils.scrollDelayTimer = setTimeout(function () {
      if (self.utils.scrollShouldBeIgnored(simpleEvent)) {
        return;
      }
      self.computedProps.handleScroll(ReactDOM.findDOMNode(self.refs.scrollable));
      self.handleScroll(self.utils.getScrollTop());
    }, self.props.scrollDelayTime);
  },
