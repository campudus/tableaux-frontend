...
const AttachmentOverlayFilter = withHandlers({
  setFilterValue: ({setFilter}) => (event) => setFilter({
    value: f.getOr("", ["target", "value"], event)
  }),
  setFilterMode: ({setFilter}) => (mode) => setFilter({
    mode
  }),
  toggleSortOrder: ({setFilter, sortOrder}) => () => setFilter({
  ...
  (props) => (
    <div className="attachment-overlay-filter-container">
      <AOFilterBar {...f.pick(["filterMode", "setFilterMode", "filterValue", "setFilterValue", "clearFilter"], props)} />
...
const AOFilterBar = compose(
  ...
  (props) => {
    const placeholder = (props.filterMode === FilterModes.CONTAINS)
    ...
    return (
      <div className="attachment-overlay-filter-bar">
        ...
        />
        <AOFilterBarModeSwitcher filterMode={props.filterMode}
...
const AOFilterBarModePopup = pure(
  ({filterMode, setFilterMode}) => (
    <div className="attachment-overlay-filter-mode-popup">
      <AOFilterModeItem setFilterMode={setFilterMode}
...
const AOFilterModeItem = ({setFilterMode, filterMode, itemMode, label}) => (
  <a href="#"
     ...
     className={(filterMode === itemMode) ? "filter-mode-item selected" : "filter-mode-item"}
  >
    {i18n.t(label)}
  </a>
);
