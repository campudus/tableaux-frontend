...
const subscribeToTable = ({cellToWatch, rowToWatch} = {}) => (WrappedComponent) => class extends Component {
  constructor(props) {
    ...
    if (f.isNil(cellToWatch) && f.isNil(rowToWatch)) {
      throw new Error("subscribeToTable(component, options) needs an options object with either a cellToWatch or rowToWatch key");
    }
  }
  ...
  isWatchedRow = row => row && row.tableId === rowToWatch.tableId && row.id === rowToWatch.tableId;

  isWatchedCell = cell => cell && cell.id === cellToWatch.id
    || (!cellToWatch.id && cell.id === `cell-${cellToWatch.tableId}-${cellToWatch.columnId}-${cellToWatch.rowId}`);

  updateData = ({cell, row}) => {
    // dont't update if the changed element is not the watched element
    if ((cellToWatch && !this.isWatchedCell(cell)) || (rowToWatch && !this.isWatchedRow(row || f.get("row", cell)))) {
      return;
    }
